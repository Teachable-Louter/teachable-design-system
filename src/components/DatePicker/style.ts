...
export const DatePickerContainer = styled.div<DatePickerContainerProps>`
  ...
  border-radius: 16px;
  box-shadow: 0px 4px 16px ${colors.alpha.shadow2};
  padding: 24px;
...
export const DayCell = styled.button<DayCellProps>`
  ...
  color: ${(props) => {
    ...
    if (props.isSelected || props.isRangeStart || props.isRangeEnd)
      return colors.text["inverse-static"];
  ...
  /* 범위 시작점 */
    ...
    css`
      ...
      &::after {
        ...
        width: 40px;
        height: 40px;
        background-color: ${colors.element.secondary};
        border-radius: 50%;
        z-index: -1;
      }
    `}
...
export const ConfirmButton = styled.button`
  ...
  &:active {
    background-color: ${colors.button["primary-fill-pressed"]};
  }
`;
